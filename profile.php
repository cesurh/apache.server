...

{
  ...
  {
    $isim_err="Kullanıcı adı en az 6 karakter olmalıdır.";
  }
  ...
  if(isset($isimd)) //if(isset($$parola2));   
  {
    ...
    $edit="UPDATE `kullanicilar` SET `kullanici_adi` = '$isimd' WHERE `kullanicilar`.`id` = ".$_SESSION["id"]."";
...

{
  ...
  if(isset($mail)) //if(isset($$parola2));   
  {
    ...
    $edit="UPDATE `kullanicilar` SET `email` = '$mail' WHERE `kullanicilar`.`id` = ".$_SESSION["id"].";";
    $calistirguncelle = mysqli_query($baglanti,$edit);
...

{
  ...
  if(isset($nmra)) //if(isset($$parola2));   
  {
    ...
    $edit="UPDATE `kullanicilar` SET `nmara` = '$nmra' WHERE `kullanicilar`.`id` = ".$_SESSION["id"].";";
    $calistirguncelle = mysqli_query($baglanti,$edit);
...

{
  ...
  if(isset($schd)) //if(isset($$parola2));   
  {
    ...
    $edit="UPDATE `kullanicilar` SET `okuladi` = '$schd' WHERE `kullanicilar`.`id` = ".$_SESSION["id"].";";
    $calistirguncelle = mysqli_query($baglanti,$edit);
...
<br>
  ...
  <li ><a class="" href="home.php"><img class="tamam"   src="users.png" style=" hover-color: red; widht: 25px; height: 30px;" id="tamam" alt="profile" /><h5>About</h5></a></li>
...
<br>
  ...
  <button type="button" name="enler" style="color: black;" class="btn btn-outline-warning">En'ler</button>
...
?>
                    <form action="profile.php?ID=<?= $id; ?>" style="background: #fff;" method="POST">
...
<p id="nameuser" style="display: none;"  
  ...
  <form action="profile.php" method="POST" style="background-color: grey;">
...
if(isset($_SESSION["username"]))
{
            $_SESSION["isimd"]=$ilgilikayit["kullanici_adi"];
            $_SESSION["mail"]=$ilgilikayit["email"];
            $_SESSION["nmra"]=$ilgilikayit["nmara"];
            $_SESSION["schd"]=$ilgilikayit["okuladi"];
}
else
{
    echo "<div class='alert alert-Danger' role='alert'><center><h3>
